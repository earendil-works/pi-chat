...
- **One VM per connected channel.** Started on `/chat-connect`, closed on `/chat-disconnect`. `/chat-spawn-all` launches one detached tmux/pi worker per configured channel using `--chat-conversation <account/channel>`. For Telegram it also launches one account dispatcher using `--chat-telegram-dispatcher <account>`; the dispatcher is the only `getUpdates` poller for that bot token and spools inbound messages to per-channel queues, so Telegram channel workers remain independent. `/chat-workers`, `/chat-open-all`, and `/chat-kill-all` manage those workers through tmux. Workers write status JSON to `~/.pi/agent/chat/worker-status/`; the `chat_workers` tool reads it.
